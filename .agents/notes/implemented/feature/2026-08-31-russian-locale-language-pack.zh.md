...
**俄语以外部客户端语言包 `@deepseek-ai/dsh-client-locale-ru` 交付。** 该包注册 `ru`（标签 `Русский`，回退 `en`），并为它所拥有的命名空间贡献俄语词典。当前这些命名空间是 `common` 与 `settings.locale`。未翻译的键以及其他命名空间按声明的回退链走到英语。该包不改动 `FALLBACK_LOCALE`，不改写 Host 偏好解析，也不在浏览器或已存储偏好指向其他已注册语言时强制使用 `ru`。
...
包级规格挂载带有内置词典的 `LocaleRuntime`，应用该包，并固定选择、已拥有键的翻译、未拥有命名空间的英语回退、`ru-RU` 浏览器采纳，以及 fiber 卸载移除该语言。`pnpm run test:gui` 在客户端套件中覆盖该包。
...
- **一个重导出全部 UI 命名空间的词典文件** — 推迟；带英语回退的部分覆盖让首个 PR 可审阅，并让后续 PR 独立扩大命名空间覆盖。

## Consequences

- 「设置 → 通用」列出 **Русский**；选择后立即切换已拥有文案，其余处在补充更多词典之前回退到英语。
- 上游 locale 与 UI 包保持可合并；俄语文案集中在一个所属包中。
- 覆盖缺口会表现为俄语外壳中的英语字符串，直到各命名空间获得俄语词典。
