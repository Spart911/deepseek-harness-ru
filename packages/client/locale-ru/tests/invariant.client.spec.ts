...
import * as LocaleRuInvariant from '@deepseek-ai/dsh-client-locale-ru/invariant'
...
describe('invariant companion', () => {
  it('reserves package ownership with an empty installer', async () => {
    ...
    await expect(ctx.plugin(LocaleRuInvariant).await()).resolves.toBeDefined()
  })

  it('has an empty node half', async () => {
    const { apply } = await import('@deepseek-ai/dsh-client-locale-ru')
